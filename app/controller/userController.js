...
let deleteUser = (req,res)=>{
    userModel.deleteOne({userId : req.body.userId}, (err, result)=>{
        if(err){
            logger.error("error deleting user details", "userController : deleteUser", 9);
            let apiResponse = response.generate(true, "error deleting user", 500, err);
            res.send(apiResponse);
        }else if(result.n == 0){
            logger.info("no user with the given user details exists", "userController : deleteUser", 8);
            let apiResponse = response.generate(true, "user not found or already deleted", 404, null);
            res.send(apiResponse);
        }else{
            logger.info("user deleted successfully", "userController : deleteUser", 9);
